...
#define HAUTEUR_ECRAN 750.0
#define LARGEUR_ECRAN 1000.0

#define HAUTEUR_PERSONNAGE 70.0
#define LARGEUR_PERSONNAGE 30.0

#define HAUTEUR_MONSTRE 70.0
#define LARGEUR_MONSTRE 30.0

#define HAUTEUR_PERSO_NN_JOUEUR 70.0
#define LARGEUR_PERSO_NN_JOUEUR 30.0

#define HAUTEUR_COFFRE 30.0
#define LARGEUR_COFFRE 30.0

#define COLLISION 1
#define PAS_COLLISION 0


int collision_combattant_ecran(combattant_t * combattant){//prend en paramètres l'image du joueur, les dimensions de l'écran + ?
    ...
    if(combattant->x + LARGEUR_PERSONNAGE  > LARGEUR_ECRAN ){
        printf("combattant y = %f\n", combattant->y);
        printf("combattant x = %f\n", combattant->x);
        printf("Il ne peut pas aller en bas car il sortirait de l'écran\n");
        return(COLLISION);
    }
    if(combattant->y + HAUTEUR_PERSONNAGE > HAUTEUR_ECRAN - 100){
        printf("combattant y = %f\n", combattant->y);
        printf("combattant x = %f\n", combattant->x);
        printf("Il ne peut pas aller en haut car il sortirait de l'écran\n");
        return(COLLISION);
...
/*
int collision_joueur_monstre(joueur_t joueur, monstre_t monstre){

    float droit_joueur = joueur.combattant->x + LARGEUR_PERSONNAGE;
    float gauche_joueur = joueur.combattant->x ;
    float bas_joueur = joueur.combattant->y + HAUTEUR_PERSONNAGE;
    float haut_joueur = joueur.combattant->y;

    float droit_monstre = monstre.combattant->x + LARGEUR_MONSTRE;
    float gauche_monstre = monstre.combattant->x ;
    float bas_monstre = monstre.combattant->y + HAUTEUR_MONSTRE;
    float haut_monstre = monstre.combattant->y;

    if(droit_joueur >= gauche_monstre && droit_joueur <= droit_monstre && haut_joueur >= bas_monstre && haut_joueur <= haut_monstre)
      return(COLLISION);
    if(droit_joueur >= gauche_monstre && droit_joueur <= droit_monstre && bas_joueur >= bas_monstre && bas_joueur <= haut_monstre)
      return(COLLISION);
    if(gauche_joueur >= gauche_monstre && gauche_joueur <= droit_monstre && haut_joueur >= bas_monstre && haut_joueur <= haut_monstre)
      return(COLLISION);
    if(gauche_joueur >= gauche_monstre && gauche_joueur <= droit_monstre && bas_joueur >= bas_monstre && bas_joueur <= haut_monstre)
      return(COLLISION);
...
int collision_combattant(combattant_t * combattant, salle_t * salle, int indice_monstre, joueur_t * joueur){
    ...
    if(combattant->type == MONSTRE){
      ...
      /*Il faut vérifier qu'il n'y ait pas de collisions entre le joueur et le monstre si le monstre est passé en paramètres*/
    }
